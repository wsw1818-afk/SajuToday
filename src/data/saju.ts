...
// 육합 (六合)
export const SIX_COMBINES: CombineClash[] = [
...
// 육충 (六沖)
export const SIX_CLASHES: CombineClash[] = [
...
export const FIVE_ELEMENTS = {
  wood:  { korean: '목', hanja: '木', color: '#4CAF50', generates: 'fire' as Element,  controls: 'earth' as Element },
  fire:  { korean: '화', hanja: '火', color: '#F44336', generates: 'earth' as Element, controls: 'metal' as Element },
  earth: { korean: '토', hanja: '土', color: '#FFC107', generates: 'metal' as Element, controls: 'water' as Element },
  metal: { korean: '금', hanja: '金', color: '#9E9E9E', generates: 'water' as Element, controls: 'wood' as Element },
  water: { korean: '수', hanja: '水', color: '#2196F3', generates: 'wood' as Element,  controls: 'fire' as Element },
};

// 일간별 특성
export const DAY_MASTER_TRAITS: Record<string, DayMasterTraits> = {
  '갑': {
...
// 헬퍼 함수들
export function getStemByKorean(korean: string): HeavenlyStem | undefined {
  return HEAVENLY_STEMS.find(s => s.korean === korean);
}

export function getBranchByKorean(korean: string): EarthlyBranch | undefined {
  return EARTHLY_BRANCHES.find(b => b.korean === korean);
}

export function getSexagenaryCycleByOrder(order: number): SexagenaryCycle | undefined {
  return SEXAGENARY_CYCLE.find(c => c.order === order);
}

export function getSexagenaryCycleByGanji(stem: string, branch: string): SexagenaryCycle | undefined {
  return SEXAGENARY_CYCLE.find(c => c.stem === stem && c.branch === branch);
}

export function getSolarTermByKorean(korean: string): SolarTerm | undefined {
  return SOLAR_TERMS.find(t => t.korean === korean);
}

/**
 * 연도별 사주 월주 기준 12절기 날짜 테이블
 * - 키: 양력 월 (1=소한, 2=입춘, 3=경칩, 4=청명, 5=입하, 6=망종,
 *               7=소서, 8=입추, 9=백로, 10=한로, 11=입동, 12=대설)
 * - 값: 해당 월에서 절기가 시작되는 날(day of month)
 * - 출처: uncle.tools (NASA DE441 + 한국천문연구원 데이터 기반)
 * - 범위: 2020~2040년
 */
export const SOLAR_TERM_DATES: Record<number, Record<number, number>> = {
  // 월: 1=소한  2=입춘  3=경칩  4=청명  5=입하  6=망종  7=소서  8=입추  9=백로  10=한로  11=입동  12=대설
  ...
  2036: { 1: 6,  2: 4,  3: 5,  4: 4,  5: 5,  6: 5,  7: 6,  8: 7,  9: 7,  10: 8,  11: 7,  12: 6  },
  2037: { 1: 5,  2: 3,  3: 5,  4: 4,  5: 5,  6: 5,  7: 7,  8: 7,  9: 7,  10: 8,  11: 7,  12: 7  },
  2038: { 1: 5,  2: 4,  3: 5,  4: 5,  5: 5,  6: 5,  7: 7,  8: 7,  9: 7,  10: 8,  11: 7,  12: 7  },
  2039: { 1: 5,  2: 4,  3: 6,  4: 5,  5: 6,  6: 6,  7: 7,  8: 8,  9: 8,  10: 8,  11: 7,  12: 7  },
  2040: { 1: 6,  2: 4,  3: 5,  4: 4,  5: 5,  6: 5,  7: 6,  8: 7,  9: 7,  10: 8,  11: 7,  12: 6  },
};
