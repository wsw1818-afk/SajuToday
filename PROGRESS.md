...
## 🚨 2026-04-18 (밤): CRITICAL 사주 계산 버그 수정 (JDN offset)

### 문제
- Council Phase 3 검증에서 QA가 발견
- `SajuCalculator.ts:43` `JDN_GANJI_OFFSET = 4` (잘못된 값)
- **모든 사주의 일주가 45칸 어긋남**
- 일주 틀림 → 일간 틀림 → 십신 전부 틀림 → 용신/운세/풀이 모두 틀림

### 원인
- 코드 주석: "2026-02-07 JDN=2461744"
- 실제 JDN(2026-02-07) = **2,461,079** (665 차이의 오타)
- 잘못된 JDN으로 offset 계산해서 4가 나왔지만, 실제 정답은 **49**

### 검증 (옵션 B - Council 1000 케이스 시뮬레이션)
| 케이스 | offset=4 (이전) | offset=49 (수정) | 정답 |
|---|---|---|---|
| 2026-02-07 | 정묘 ❌ | **임자** ✅ | 임자 |
| 1980-05-15 | 계묘 ❌ | **무자** ✅ | 무자 |
| 1990-12-20 | 갑술 ❌ | **기미** ✅ | 기미 |
| 2000-03-10 | 임오 ❌ | **정묘** ✅ | 정묘 |
| 60일 주기 | - | **임자→임자** ✅ | OK |
| 1년 분포 (60갑자) | - | **균등 (평균 6.08회)** | OK |

### 수정
- `SajuCalculator.ts:43` `JDN_GANJI_OFFSET = 4` → **`= 49`**
- 주석 정정 (실제 JDN 값 + 검증 케이스 4개 명시)

### 영향 범위
- 모든 사용자의 사주 계산 결과 정상화
- 일주 → 일간 → 십신 → 용신 → 운세 점수 → AI 텍스트 매칭 모두 정확해짐
- v1plus 텍스트 데이터는 그대로지만 키 매칭이 올바른 사주 기반으로 됨

---

## 📋 2026-04-18 (저녁): Phase 1+2+3 — 운세 다양성 + 명리학 깊이 + 출시 준비

### Phase 1: 클로징/시작 표현 다양화 (Council 검증 통과)
- 13개 반복 패턴 (603회) → 6~10가지 변형으로 분산
- "걱정하지 마세요" 89회/"좋은 결과가 있을 거예요" 57회/"예상치 못한" 257회 → 모두 0회
- "~날이에요" 클리셰 258회 → 0회
- 신살 prefix (천을귀인/도화/역마) 각 8가지 변형 풀 + dateHash 분산
- Council 핫픽스: "낯선 결말가" 등 조사 오류, "뜻밖의" 117회 폭증 모두 해결
- 검증 스크립트 강화 (신 클리셰 폭증 자동 탐지, 50~60회 임계값)

### Phase 2: 일주 60갑자 도입부 추가 (Council 검증 통과)
- `src/data/generated/ilju_intros.json` 신규: **60일주 × 5변형 = 300개 도입부**
- `generatePersonalNarrative.ts`: 본문 앞에 일주 도입부 prepend + dateHash 분산
- `useTodayFortune.ts`: `myIlju` 파라미터 전달
- Council 핫픽스: 종결어미 "결이에요/흐름이에요" 60회 수렴 → 5가지 분산
- 같은 일간 6일주 시작 분산 ("큰 나무가 ~" → 다양한 시작)
- 같은 사용자도 매일 5가지 회전 → 30일이면 6번 반복

### Phase 3: 명리학 깊이 + 출시 준비 — Council 검증 후 롤백 (정직 모드)

**Phase 3 시도:**
- 희신/구신 강도 표시 추가
- 12운성 카테고리 보정 (`getStageNote`)
- AnalyticsService 이벤트 5개 type 추가

**Council 검증 결과 (PM + Engineer + Devils 만장일치):**
- ❌ Analytics 5개 이벤트: type만 추가, **실제 호출 0건** → CLAUDE.md "구조만 만들고 데이터 안 채우는 것 금지" 규칙 위반 ("오늘의 고민" 위젯과 동일 패턴)
- ❌ 12운성 카테고리 보정: 사용자 일주는 고정 → 같은 운성 시기에 매일 같은 한 줄 = "결정박스 시즌 2" 함정 위험
- ❌ 희신/구신 명리학 용어 표시: 일반 사용자 인지 부담 (오늘의 고민 위젯 "탭 의미 모르겠다"와 동일)
- ❌ Release 키스토어 미생성: "출시 준비" 라벨 거짓 선언

**A안 정직 모드 적용 (Council 권고 100% 수용):**
- ✅ 12운성 카테고리 보정 제거 (`getStageNote` 함수 + 호출 삭제)
- ✅ 희신/구신 표시 제거
- ✅ Analytics 5개 신규 type 삭제 (호출 없는 type = 거짓말)
- ⏸️ 출시는 사용자가 나중에 하기로 결정

**남은 자산:**
- ✅ Phase 1 (클로징/시작 표현 다양화): 유지 (검증 통과)
- ✅ Phase 2 (일주 60갑자 도입부 300개): 유지 (검증 통과)
- ✅ 신살 prefix 8가지 변형: 유지 (검증 통과)
- ✅ 명리학 근거 1줄 표시 + AI 정직 공개: 유지

**다음 Phase 결정 조건 (Devils 강력 권고):**
- 출시 → 사용자 데이터 수집 (최소 4주)
- 데이터 기반으로만 다음 작업 결정
- "Council 합의" 단독으로는 새 기능 추가 금지

---

## 📋 2026-04-18 (오후): "오늘의 결정 박스" 제거 + 본문 압축 (사용자 신고 대응)

### 사용자 신고
> "결정박스는 계속 똑같은 게 나와 무슨 의미인지 모르겠다, 본문은 너무 길어 다 들어오지 않는다"

### "오늘의 결정 박스" 제거 (Phase A.5에서 도입한 기능 롤백)
- **본질적 결함**: 폴백 매트릭스(10×3×4=120종)가 같은 사주 입력에 항상 같은 출력 → 매일 똑같은 텍스트 반복
- **Council-Code 4명 검토**: User/Engineer/PM/Devils 중 Engineer만 즉시 삭제 권고, 3명은 폴백 다양화 권고. 사용자 추가 설명("계속 똑같은게 나온다")으로 본질적 결함 확인 → 삭제 결정
- **제거 범위**:
  - `src/components/DecisionBoxView.tsx` 파일 전체 삭제
  - `src/services/generatePersonalNarrative.ts`: `DecisionBox` 인터페이스, `getDecisionBox`, `TENGOD_ACTIONS`, `TENGOD_REDS`, `getStageTone`, `buildFallbackDecisionBox` (총 ~170줄)
  - `src/hooks/useTodayFortune.ts`: import + interface 필드 + return 필드
  - `src/screens/DailyFortuneScreen.tsx`: import + 조건부 렌더 블록
- **재도입 조건**:
  - 매일 다른 텍스트가 보장되는 데이터 소스 (단순 매트릭스 X)
  - AI v2 데이터의 `decisions` 섹션이 카테고리당 100개 이상 변형 보유

### 본문 압축
- **렌더링 절단** (`DailyFortuneScreen.tsx`): `trimSentences` 헬퍼 추가
  - detail: 첫 2문장 / 최대 150자
  - 카테고리(💰💕💼🏃): 각 첫 1문장 / 최대 70자
  - 데이터 자체는 보존 (공유 카드 등 다른 용도 위해)
- **postprocess_v1.py 추가 필터** (`remove_filler` 함수): User Council 발견 패턴 제거
  - 도입 추임새 (가만히 살피면, 슬며시, 결이 다르게 등)
  - 반복 위로구 (너무 걱정 마세요, 오전만 잘 넘기면)
  - 메모/기록 강요 문장 전체 (한 줄 적어두, 간단히 기록해 등)
  - 결말 격려 (행운을 빌어요, 멋지게 헤쳐나가세요)
  - 군더더기 부사 (혹시, 특히)
- **결과**: 화면 표시 텍스트 1,000~1,260자 → **약 350~450자** (70% 감소)

---

## 📋 2026-04-18 (오전): 운세 텍스트 후처리 (Phase A.5) + "오늘의 고민" 위젯 제거

### Phase A.5: v1 텍스트 후처리 재시공
- **배경**: Council-Code 검수 결과 v1 텍스트의 4종 세트 표현 (점심때 56.9% / 주변사람 53.3% / 오후3시 49% / 긍정마음 42.9%) 발견
- **1차 시도 실패**: 정규식 치환 후 깨진 문법 280건 발생 ("한낮에는쯤", "흐름가" 등)
- **재시공 결과** (`scripts/postprocess_v1.py`):
  - 깨진 문법 280건 → **0건** (조사 안전 처리)
  - 첫 문장 "오늘" 편중 48.3% → **17.9%** (다양화 7종 패턴)
  - 신 클리셰 모두 10% 미만 (8~12종 후보 풀)
  - 4종 세트 모두 0건 유지
- **검증 강화** (`scripts/validate_narratives.py`): 깨진 문법 + 첫문장 분포 자동 검출
- **결과**: `narratives_generated_v1plus.json` (485KB, 모든 검증 통과)
- **결정박스 a11y**: 44px 터치 타겟, accessibilityLabel, 색상 대비 강화

### "오늘의 고민" 위젯 제거 (Phase 1-3 롤백)
- **사용자 신고**: "탭 의미 모르겠다 + 선택해도 반응 없다 + 운세 패턴 비슷하다"
- **Council-Code 7명 검토 결과** (PM/Engineer/User/Designer/QA/DevOps/Devils 만장일치):
  - **Engineer 발견**: `useFocusEffect`가 매 포커스마다 concern 덮어씀 (선택→0.1초 후 복원되는 진짜 원인)
  - **반쪽 구현**: `getConcernWeights` 함수 정의만 되고 호출 0회 (CLAUDE.md "구조만 만들고 데이터 안 채우는 것 금지" 규칙 위반)
  - 운세 앱은 **수동 소비**가 본질, 매일 입력 강요는 UX 위반
  - 4개 카테고리 = 5가지 순열만 → 5일이면 다 봄 → "패턴 비슷" 신고 강화
- **제거 범위**:
  - DailyFortuneScreen에서 위젯/모달/state/handleConcernSelect 제거
  - `comprehensiveReading` 고정 순서 (재물→연애→직장→건강) + 카테고리 라벨(💰💕💼🏃) 추가 (디자이너 권고 — 시각적 구분 강화)
  - `topCategoryEmoji='✨'`, `topCategoryText=wealth` 고정 (ShareCard 의존성)
  - import 라인 + 스타일 8개 제거
- **보존**:
  - `src/services/ConcernService.ts` 파일 (재사용 자산, PM 권고)
  - AsyncStorage stale 데이터 (`today_concern`) 자동 만료에 위임 (DevOps 권고)
- **재도입 조건**:
  - 사용자가 카테고리 선택 시 운세 풀이 텍스트가 실제로 달라지는 것이 화면에서 보이는 경우
  - `getConcernWeights`가 실제 점수 계산에 연동되는 경우
  - AnalyticsService에 `concern_selected` / `concern_session_changed` 이벤트가 측정되는 경우
